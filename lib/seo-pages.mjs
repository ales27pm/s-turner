...
function naturalList(values) {
  ...
  if (values.length === 2) return values.join(' et ');
  return `${values.slice(0, -1).join(', ')} et ${values.at(-1)}`;
}

function modelTypeLabel(model) {
  if (model.type === 'Plain-pied') return 'maison de plain-pied';
  if (model.type === 'Deux étages') return 'maison à deux étages';
  return model.type.toLocaleLowerCase('fr-CA');
}

function modelMetaDescription(model) {
  const intro = `Découvrez ${model.name}, ${modelTypeLabel(model)} de style ${model.style.toLocaleLowerCase('fr-CA')}, ${areaFormatter.format(model.area)} pi², ${plural(model.bedrooms, 'chambre')} et ${plural(model.bathrooms, 'salle de bain', 'salles de bain')}${model.garage ? ', avec garage' : ''}.`;
  const features = model.features.map(feature => feature.toLocaleLowerCase('fr-CA'));
  for (let count = features.length; count > 0; count -= 1) {
    const candidate = `${intro} Atouts : ${naturalList(features.slice(0, count))}.`;
    if (candidate.length <= 165) return candidate;
  }
  return metadataDescription(`${intro} Un plan personnalisable fabriqué au Québec par Maisons S. Turner.`, 165);
}

function modelConfiguration(model) {
  const levels = model.floors === 1 ? 'sur un niveau' : `sur ${model.floors} niveaux`;
  return `${model.name} développe ${areaFormatter.format(model.area)} pi² ${levels}, avec ${plural(model.bedrooms, 'chambre')} et ${plural(model.bathrooms, 'salle de bain', 'salles de bain')}${model.garage ? ' ainsi qu’un garage' : ''}.`;
}

function modelFeatureSummary(model) {
  const features = model.features.map(feature => `« ${feature} »`);
  return `Les points forts publiés pour ${model.name} sont ${naturalList(features)}.`;
}

function featureDetail(model, feature) {
  return model.featureDetails?.find(item => item.title === feature)?.copy
    || 'Cet élément fait partie des caractéristiques publiées dans la fiche officielle.';
}
...
export function faviconLinks(publicOrigin) {
  return `<link rel="icon" href="${escapeHtml(publicUrl(publicOrigin, '/favicon.ico'))}" sizes="any">
  <link rel="icon" type="image/png" sizes="32x32" href="${escapeHtml(publicUrl(publicOrigin, '/favicon-32x32.png'))}">
  <link rel="apple-touch-icon" sizes="180x180" href="${escapeHtml(publicUrl(publicOrigin, '/apple-touch-icon.png'))}">`;
}
...
function pageHead({ title, description, canonical, image, type = 'website', structuredData, indexable = true }) {
  ...
  ${faviconLinks(new URL(canonical).origin)}
...
function modelCard(model, publicOrigin, headingLevel = 2) {
  ...
  return `<article class="model-card">
    ...
    <div class="model-card-body">
      ...
      <div class="model-card-specs"><span>${areaFormatter.format(model.area)} pi²</span><span>${plural(model.bedrooms, 'chambre')}</span><span>${plural(model.bathrooms, 'salle de bain', 'salles de bain')}</span></div>
...
function renderPlanSection(model) {
  if (!model.planLevels?.length) return '';
  const levels = model.planLevels.map((level, levelIndex) => {
    const headingId = `plan-${model.id}-${levelIndex}`;
    const rooms = level.rooms.map(room => `<div><dt>${escapeHtml(room.name)}</dt><dd>${escapeHtml(room.dimensions)}</dd></div>`).join('');
    return `<section class="plan-level" aria-labelledby="${headingId}"><h3 id="${headingId}">${escapeHtml(level.name)}</h3><dl class="room-list">${rooms}</dl></section>`;
  }).join('');
  return `<section class="shell plan-section" aria-labelledby="plan-layout-title"><h2 id="plan-layout-title">Pièces et dimensions du modèle ${escapeHtml(model.name)}</h2><p>Les dimensions ci-dessous proviennent du plan publié pour ${escapeHtml(model.name)}. Elles servent de référence et doivent être confirmées avec l'équipe Turner lors de la personnalisation.</p><div class="plan-levels">${levels}</div></section>`;
}
...
export function renderModelPage({ model, content, publicOrigin, indexable = true }) {
  ...
  const description = modelMetaDescription(model);
  ...
  const structuredData = [
    ...
    breadcrumbNode(breadcrumbs, canonical),
    {
      ...
      additionalProperty: [
        ...
        { '@type': 'PropertyValue', name: 'Points forts', value: model.features.join(', ') },
      ],
    },
  ];
  ...
  <main id="contenu">
    ...
    <section class="shell model-details" aria-labelledby="model-overview-title"><div class="model-copy"><h2 id="model-overview-title">Portrait du modèle ${escapeHtml(model.name)}</h2><div class="model-narrative"><p>${escapeHtml(model.sourceDescription || model.description)}</p><p>${escapeHtml(modelConfiguration(model))}</p><p>${escapeHtml(modelFeatureSummary(model))}</p></div><ul class="feature-list">${model.features.map(feature => `<li><strong>${escapeHtml(feature)}</strong><span>${escapeHtml(featureDetail(model, feature))}</span></li>`).join('')}</ul></div><aside class="spec-panel" aria-labelledby="spec-title"><h2 id="spec-title">Caractéristiques</h2><dl class="spec-list"><div><dt>Superficie</dt><dd>${areaFormatter.format(model.area)} pi²</dd></div><div><dt>Chambres</dt><dd>${model.bedrooms}</dd></div><div><dt>Salles de bain</dt><dd>${model.bathrooms}</dd></div><div><dt>Étages</dt><dd>${model.floors}</dd></div><div><dt>Garage</dt><dd>${model.garage ? 'Oui' : 'Non'}</dd></div><div><dt>Style</dt><dd>${escapeHtml(model.style)}</dd></div></dl><p class="source-note">Données de la fiche Turner${sourceDate ? `, mise à jour le ${escapeHtml(sourceDate)}` : ''}. <a href="${escapeHtml(model.sourceUrl)}" target="_blank" rel="noreferrer">Consulter la source officielle</a></p></aside></section>
    <section class="feature-section"><div class="shell"><h2>Adapter ${escapeHtml(model.name)} à votre projet</h2><p>Ces caractéristiques constituent le point de départ publié pour ${escapeHtml(model.name)}. L'équipe Turner peut confirmer les adaptations réalisables, les inclusions et les coûts selon le terrain et le devis.</p></div></section>
